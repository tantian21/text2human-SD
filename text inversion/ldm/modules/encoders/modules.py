...
class BERTEmbedder(AbstractEncoder):
    ...
    def forward(self, text, embedding_manager=None):
        self.transformer=self.transformer.to(device=torch.device("cuda"))
        if self.use_tknz_fn:
...
class FrozenCLIPEmbedder(AbstractEncoder):
    ...
    def forward(self, text, embedding_manager=None):
        batch_encoding = self.tokenizer(text, truncation=True, max_length=self.max_length, return_length=True,
                                        return_overflowing_tokens=False, padding="max_length", return_tensors="pt")
        tokens = batch_encoding["input_ids"].to(self.device)
        outputs = self.transformer(input_ids=tokens,embedding_manager=embedding_manager)

        z = outputs.last_hidden_state
        return z

    def encode(self, text, embedding_manager=None):
        return self(text, embedding_manager=embedding_manager)
